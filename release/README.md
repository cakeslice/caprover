...
2. Create `release/X.Y.Z` from the desired commit on `master`.
...
`release` to `master` backmerge PR and enables auto-merge when possible.

The release branch captures the state of `master` when `release/X.Y.Z` was
created. Later commits to `master` are left for the next release.

## Hotfix

1. Create `hotfix/X.Y.Z` from the current `release` branch.
2. Apply the fix.
